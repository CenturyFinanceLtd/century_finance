...
function TagPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setError('');
      try {
        const json = await api.getJson(`/api/blogs?tag=${encodeURIComponent(decodedTag)}`);
        setBlogs(json);
      } catch (e) {
        setError(e.message);
      } finally {
  ...
  return (
    <div>
      <PageTitle title={`Tag: ${decodedTag}`} />
      <section className="tf-blog">
        <div className="tf-container">
          <div className="row">
            {loading && <div className="col-12"><p>Loading...</p></div>}
            {error && !loading && <div className="col-12"><p style={{color:'red'}}>Error: {error}</p></div>}
                ...
                <article className="tf-blog-item">
                  <div className="image">
                    <Link to={`/blog/${b.slug}`}>
                      {b.coverImageUrl ? (
                        ...
                        <div style={{width:'100%',height:'240px',background:'#eee',display:'flex',alignItems:'center',justifyContent:'center'}}>No Image</div>
                      )}
                    </Link>
                  ...
                  <div style={{marginTop:'8px'}}>
                    {(b.tags || []).slice(0,4).map((t) => (
                      <Link key={t} to={`/blogs/tag/${encodeURIComponent(t)}`} style={{marginRight: 8, fontSize: 12}}># {t}</Link>
                    ))}
                  </div>
                </article>
              </div>
